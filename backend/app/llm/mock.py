...
class MockLLMProvider:
    ...
    _ACTION_VERB = re.compile(
        r"\b(add|alert|bound|configure|create|decrease|define|document|enable|enforce|increase|"
        r"implement|limit|reduce|remove|set|test|use|validate)\b",
        re.IGNORECASE,
    )
    _SPLIT = re.compile(
        r"\s*(?:;|\s+(?:and|&)\s+)(?=(?:add|alert|bound|configure|create|decrease|define|"
        r"document|enable|enforce|increase|implement|limit|reduce|remove|set|test|use|validate)\b)",
        re.IGNORECASE,
    )
